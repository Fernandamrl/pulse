...
// Run: npm run build
// Standalone n8n Code node artifact for PULSE ADS v6.
...
function __pulseRunLegacy() {
  // ============================================================================
  // PROCESSAR SNAPSHOT ADS — PULSE V5
  // East River Monitor — classificação por meta_latency_formatada
  //
  // Monitora todas as filas do report ADS com backlog > 0.
  // Status por fila:
  ...
  var FRIENDLY_NAMES = {
    ...
    '7963': 'Eff. Unit (7963)',
    '8813': 'Recall BR (8813)',
    '9840': 'Inspection (9840)',
    '13169': 'Live Slicin (13169)',
    '10801': 'Adv. Verif. BR (10801)',
    '14198': 'Recorded&Live (14198)',
    '7587': 'Material Rep (7587)',
    '9693': 'Material Comp (9693)',
    '11149': 'Recall AIGC (11149)',
    '8816': 'Recall MX (8816)',
    '3922': 'Adv. Verif. BR Initial (3922)',
    '4118': 'Adv. Verif. AM (4118)',
    '11145': 'Eff. Adv. AIGC (11145)',
    '9684': 'Contract BR (9684)',
    '8797': 'Contract Unit BR (8797)'
  };
  ...
  // ============================================================================
  // TEXTO WHATSAPP — PULSE ADS V5
  // Exibe todas as filas com backlog > 0.
  // ============================================================================
  
  var lines = [];
  
  lines.push('*📡 PULSE | EAST RIVER ADS*');
  ...
  lines.push('Filas ativas: *' + activeQueues.length + '* | 🔴 ' + estourados.length + ' | 🟠 ' + alertas.length + ' | 🟢 ' + (activeQueues.length - estourados.length - alertas.length));
  lines.push('');
  lines.push('*FILAS COM BACKLOG*');
  
  for (var i = 0; i < activeQueues.length; i++) {
    lines.push(compactQueueLine(activeQueues[i]));
  }
  
  if (activeQueues.length === 0) {
    lines.push('🟢 Nenhuma fila ADS com backlog.');
  }
...
return PULSE_CORE.executive.enhance(__pulseLegacyResult, $input.first().json, { reportCode: "ADS" });